...
func _on_physics_process(_delta : float) -> void:
	...
	var desired_velocity := target_direction * speed
	
	if navigation_agent_2d.avoidance_enabled:
		navigation_agent_2d.velocity = desired_velocity
	else:
		apply_velocity(desired_velocity)

func on_safe_velocity_computed(safe_velocity: Vector2) -> void:
	apply_velocity(safe_velocity)

func apply_velocity(v: Vector2) -> void:
	current_velocity = current_velocity.lerp(v, 0.15)

	if abs(current_velocity.x) > 0.05:
		animated_sprite_2d.flip_h = current_velocity.x < 0
